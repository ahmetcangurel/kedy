...
const importAnalyzer_1 = require("../utils/importAnalyzer");
const fixImports = (options) => {
    var _a, _b;
    const spinner = (0, nanospinner_1.createSpinner)("Analyzing imports...").start();
    try {
        const config = (0, configLoader_1.loadConfig)();
        const { importAnalysis } = config;
        // Analiz yap
        const result = (0, importAnalyzer_1.analyzeProject)(config);
        ...
        // Detaylı rapor
        if ((_a = options.detailed) !== null && _a !== void 0 ? _a : importAnalysis.detailedReport) {
            console.log("\n🔍 Detailed Report:");
            result.details.forEach((fileInfo) => {
                const relativePath = fileInfo.filePath
                    .replace(process.cwd(), "")
                    .substring(1);
                console.log(`\n📁 ${relativePath}`);
                fileInfo.unusedImports.forEach((importInfo) => {
                    console.log(`   ❌ Line ${importInfo.line}: ${importInfo.items.join(", ")} from '${importInfo.source}'`);
                });
            });
        }
        // Otomatik düzeltme
        if ((_b = options.autoFix) !== null && _b !== void 0 ? _b : importAnalysis.autoFix) {
...
const fixFileImports = (fileInfo) => {
    try {
        const content = fs_1.default.readFileSync(fileInfo.filePath, "utf8");
        ...
        fileInfo.unusedImports.forEach((importInfo) => {
            ...
            const defaultImportMatch = originalLine.match(/import\s+(\w+)\s+from\s+['"]([^'"]+)['"]/);
            ...
            const namedImportMatch = originalLine.match(/import\s*{([^}]+)}\s+from\s+['"]([^'"]+)['"]/);
            if (namedImportMatch) {
                const allItems = namedImportMatch[1]
                    .split(",")
                    .map((item) => item.trim());
                const remainingItems = allItems.filter((item) => !importInfo.items.includes(item));
                if (remainingItems.length === 0) {
                    ...
                    const newImport = `import { ${remainingItems.join(", ")} } from '${namedImportMatch[2]}'`;
            ...
            const mixedImportMatch = originalLine.match(/import\s+(\w+)\s*,\s*{([^}]+)}\s+from\s+['"]([^'"]+)['"]/);
            if (mixedImportMatch) {
                const defaultItem = mixedImportMatch[1];
                const allNamedItems = mixedImportMatch[2]
                    .split(",")
                    .map((item) => item.trim());
                const remainingNamedItems = allNamedItems.filter((item) => !importInfo.items.includes(item));
                if (remainingNamedItems.length === 0) {
                    // Sadece default import kalır
                    lines[lineIndex] = `import ${defaultItem} from '${mixedImportMatch[3]}'`;
                }
                else {
                    // Hem default hem named import'lar kalır
                    const newImport = `import ${defaultItem}, { ${remainingNamedItems.join(", ")} } from '${mixedImportMatch[3]}'`;
                    lines[lineIndex] = newImport;
                }
                modified = true;
            }
        ...
        logger_1.logger.error(`Error fixing file ${fileInfo.filePath}: ${error}`);
        return false;
    }
};
