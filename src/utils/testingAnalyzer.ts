...
const analyzeTestCoverage = (
  ...
  const testableFiles = allFiles.filter(
    (file) =>
  ...
  testableFiles.forEach((file) => {
    ...
    const hasTest = testFiles.some((testFile) => {
      ...
      return (
        ...
        testFileName.includes(
          fileName.replace(".jsx", "").replace(".tsx", "")
        ) ||
...
const analyzeTestQuality = (
  ...
  testFiles.forEach((testFile) => {
    try {
      ...
      if (!mockMatches) {
        issues.push(
          `${relativePath}: Consider using mocks for external dependencies`
        );
        score -= 3;
      }
      ...
      const setupMatches = content.match(
        /(beforeEach|afterEach|beforeAll|afterAll)/g
      );
      if (!setupMatches) {
...
const analyzeTestPatterns = (
  ...
  if (
    !frameworks.includes("React Testing Library") &&
    !frameworks.includes("React Native Testing Library")
  ) {
    recommendations.push(
      "Consider using React Testing Library for component testing"
    );
  }
...
// Testing analizi yap
export const analyzeTesting = (allFiles: string[]): TestingMetrics => {
  const testFiles = allFiles.filter(
    (file) =>
      file.includes(".test.") ||
      file.includes(".spec.") ||
